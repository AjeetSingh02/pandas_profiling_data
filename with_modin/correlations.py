...
config_corr = {
    ...
    "correlations": {'pearson': True, 'spearman': True, 'kendall': True, 'phi_k': True, 'cramers': True, 'recoded': True},
...
def categorical_matrix(
    ...
    categoricals = {
        ...
        <= config_corr["categorical_maximum_correlation_distinct"]
    }
...
def calculate_correlations(df: 'modin pandas DataFrame', bdf: 'pandas dataframe', variables: dict) -> dict:
    ...
    Args:
        ...
        df: modin DataFrame with variables.
		bdf: pandas DataFrame with variables.
    Returns:
    ...
    for correlation_name in ["pearson", "spearman", "kendall"]:
        if config_corr["correlations"][correlation_name]:
            try:
    ...
    if config_corr["correlations"]["phi_k"]:
        import phik
        ...
        with warnings.catch_warnings():
            ...
            for col in df.columns.tolist():
                try:
                    ...
                    if pd.api.types.is_numeric_dtype(df[col]):
                        intcols.append(col)
                        selcols.append(col)
                    elif df[col].nunique() <= config_corr["categorical_maximum_correlation_distinct"]:
                        selcols.append(col)
                except TypeError:
                    continue
                except ValueError:
            ...
            try:		
                try:
                    ...
                    correlations["phi_k"] = bdf[selcols].phik_matrix(interval_cols=intcols)

                # Only do this if the column_order is set
                with suppress(KeyError):
                    # Get the preferred order
                    column_order = config_corr["column_order"]
...
def perform_check_correlation(
    ...
    correlation_overrides = config_corr["correlation_overrides"]
